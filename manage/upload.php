<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['destnode']) {
		...
		if (!file_exists($src)) {
			die("Impossible d'acc�der au fichier � copier.");
		}
		if (file_exists($dest)) {
			die("Le fichier existe d�j�.");
		}
		if (!copy($src, $dest)) {
			die("Echec de copie.");
		}
...
<body>
	...
	<p><label for="destnode">D�placer vers</label>
...
?>
	...
	<p id="submit"><input type="submit" value="Ins�rer le fichier"></p>
	</fieldset>
	</form>
</body>
</html>
